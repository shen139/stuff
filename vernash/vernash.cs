...
namespace Vernash
{
    class vernash
    {
        private int BLOCKS = 250;
        private int EXPANSION = 2;
        private int RANDOM_EXPANSION = 2;

        public static string binFileHeader = "VERNASH";

        private int MAX_OUTPUT_LENGTH = 100000; //100k
         ...
         */
        private int[] genKeyPos(int nPos, string key)
        {
         ...
         * 
         */
        public string unScrambleText(string text, string key)
        {
            char[] scrambled = new char[text.Length];
        ...
        public string vernashCrypt(byte[] text, string key, bool saveToFile, string filename)
        {
            ...
            if (saveToFile)
            {
                try
                {
                    ...
                    bw.Write(System.Text.Encoding.UTF8.GetBytes(binFileHeader));
                }
                catch (Exception e)
                {
            ...
            for (int p = 0; p < text.Length; p += BLOCKS)
            {
                ...
                exText = scrambleText(exText, key);
                ...
                else
                {
                    ...
                    res += new string(crtText, 0, textLen);
                }
            }
        ...
        public byte[] vernashDeCrypt(string text, string key, bool saveToFile, string filename)
        {
            ...
            for (int p = 0; p < text.Length; p += BLOCKS * EXPANSION * RANDOM_EXPANSION)
            {
                ...
                redString = reduceString(unScrambleText(new string(crtText, 0, textLen), key));
                if (redString == null)
                {
                    return null;
                }
